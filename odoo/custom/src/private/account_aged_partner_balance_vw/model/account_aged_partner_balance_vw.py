...
class account_aged_partner_balance_vw(osv.TransientModel):
    ...
    _columns = {
        ...
        'days_due_01to30': fields.float(u'Period 1'),
        'days_due_31to60': fields.float(u'Period 2'),
        'days_due_61to90': fields.float(u'Period 3'),
        'days_due_91to120': fields.float(u'Period 4'),
        'days_due_121togr': fields.float(u'> Period 4'),
...
class account_aged_trial_balance(osv.TransientModel):
    ...
    def _get_partners(self, cr, uid, ids, form, context=None):
        ...
        extra_table = ''
        extra_cond = ''
        if wzd_brw.salesperson_id:
            extra_table = ', account_invoice ai'
            extra_cond = 'AND am.id = ai.move_id AND ai.user_id = %s ' % (wzd_brw.salesperson_id.id)
        move_state = ['draft', 'posted']
        if self.target_move == 'posted':
            move_state = ['posted']
        cr.execute('SELECT DISTINCT res_partner.id AS id,\
                    res_partner.name AS name \
                FROM res_partner,account_move_line AS l, account_account aa, account_move am \
                    ' + extra_table + ' \
                WHERE (l.account_id=aa.id) \
                    ' + extra_cond + ' \
                    AND (l.move_id=am.id) \
                    AND (am.state IN %s)\
                    AND (aa.type IN %s)\
    ...
    def _get_lines(self, cr, uid, ids, form, context=None):
        ...
        cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit) \
                    ' + type_query + '\
                    FROM account_move_line AS l, account_account aa, account_move am \
                    ' + extra_table + ' \
    ...
    def _get_invoice_by_partner(self, cr, uid, ids, rp_ids, context=None):
        ...
        for rp_brw in rp_brws:
            ...
            for inv_brw in inv_obj.browse(cr, uid, inv_ids):
                ...
                today = datetime.strptime(wzd_brw.date_from, '%Y-%m-%d')
    ...
    def _get_lines_by_partner_without_invoice(
        ...
        for line in acc_move_line.browse(cr, uid, move_lines_ret, context=context):
            date_due = datetime.strptime(line.date, '%Y-%m-%d')
