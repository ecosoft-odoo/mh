...
class sale_team(osv.osv):
    ...
    _columns = {
        ...
        "require_paid": fields.boolean(
            "Require Paid Invoice", help="Require invoice to be paid in full amount."
        ),
...
class commission_worksheet(osv.osv):
    ...
    def _get_other_info(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for sheet in self.browse(cr, uid, ids, context):
            ...
            if object:
                res[sheet.id] = {
                    "commission_rule_id": object.commission_rule_id
                    and object.commission_rule_id.id
                    or False,
    ...
    def _calculate_commission(self, cr, uid, rule, worksheet, invoices, context=None):
        if rule.type == "percent_fixed":
            return self._calculate_percent_fixed(
                cr, uid, rule, worksheet, invoices, context=context
            )
        if rule.type == "percent_product_category":
            return self._calculate_percent_product_category(
                cr, uid, rule, worksheet, invoices, context=context
            )
        if rule.type == "percent_product":
            return self._calculate_percent_product(
                cr, uid, rule, worksheet, invoices, context=context
            )
        if rule.type == "percent_product_step":
            return self._calculate_percent_product_step(
                cr, uid, rule, worksheet, invoices, context=context
            )
        if rule.type == "percent_amount":
            return self._calculate_percent_amount(
                cr, uid, rule, worksheet, invoices, context=context
            )
        # No matched rule return False as signal.
        return False

    def _prepare_worksheet_line(
        self, worksheet, invoice, base_amt, commission_amt, context=None
    ):
        # Invoice or Refund
    ...
    def _get_percent_fixed(self, cr, uid, rule):
        percent_commission = rule.fix_percent
        return percent_commission

    def _calculate_percent_fixed(
        self, cr, uid, rule, worksheet, invoices, context=None
    ):
        if context is None:
            context = {}
        context.update({"last_loop": False})
        commission_rate = self._get_percent_fixed(cr, uid, rule) / 100
        ...
        for invoice in invoices:
            ...
            res = self._prepare_worksheet_line(
                worksheet, invoice, base_amt, commission_amt, context=context
            )
    ...
    def _calculate_percent_product_category(
        self, cr, uid, rule, worksheet, invoices, context=None
    ):
        if context is None:
            context = {}
        ...
        for invoice in invoices:
            ...
            for line in invoice.invoice_line:
                percent_commission = self._get_percent_product_category(cr, uid, line)
    ...
    def _get_percent_product(self, cr, uid, line):
        percent_commission = (
            (line.price_unit >= line.product_id.limit_price)
            and line.product_id.percent_commission
            or 0.0
        )
        return percent_commission

    def _calculate_percent_product(
        self, cr, uid, rule, worksheet, invoices, context=None
    ):
        if context is None:
            context = {}
        ...
        for invoice in invoices:
            ...
            commission_amt = 0.0
            for line in invoice.invoice_line:
                percent_commission = self._get_percent_product(cr, uid, line)
    ...
    def _get_percent_product_step(self, cr, uid, line):
        ...
        if product:
            ...
            if product.rate_step_ids:
                rate_steps = [
                    (x.amount_over, x.percent_commission) for x in product.rate_step_ids
                ]
    ...
    def _calculate_percent_product_step(
        self, cr, uid, rule, worksheet, invoices, context=None
    ):
        if context is None:
            context = {}
        ...
        for invoice in invoices:
            ...
            for line in invoice.invoice_line:
                ...
                percent_commission = self._get_percent_product_step(cr, uid, line)
                commission_rate = percent_commission and percent_commission / 100 or 0.0
                if commission_rate:
                    commission_amt += line.price_subtotal * commission_rate
            res = self._prepare_worksheet_line(
    ...
    def _get_percent_amount(self, cr, uid, range):
        percent_commission = range.percent_commission
        return percent_commission

    def _calculate_percent_amount(
        self, cr, uid, rule, worksheet, invoices, context=None
    ):
        if context is None:
            context = {}
        ...
        for invoice in invoices:
            ...
            for range in ranges:
                commission_rate = self._get_percent_amount(cr, uid, range) / 100
                if base_amt <= range.amount_upto:
                    commission_amt = base_amt * commission_rate
                    break
            res = self._prepare_worksheet_line(
                worksheet, invoice, commission_amt, context=context
            )
            worksheet_line_obj.create(cr, uid, res, context=context)
        return True
    ...
    def _get_product_commission(self, cr, uid):
        ...
        product = ir_model_data.get_object(
            cr, uid, "sale_commission_calc", "product_product_commission"
        )
        return product.id

    def _get_worksheet(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get("commission.worksheet.line").browse(
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        if ("last_loop" not in context) or context.get("last_loop", False):
            for worksheet in self.browse(cr, uid, ids, context=context):
                ...
                ).update_commission_line_status(cr, uid, line_ids, context=context)
                # Start calculation.
    ...
    _columns = {
        ...
        "invoice_ids": fields.one2many(
            "account.invoice", "worksheet_id", "Invoices", readonly=True
        ),
        "amount_draft": fields.function(
            _amount_all,
            ...
            store={
                ...
                "commission.worksheet.line": (
                    _get_worksheet,
                    ["done", "force", "adjust_amt", "commission_state"],
                    10,
                ),
            },
        ),
        "amount_valid": fields.function(
            _amount_all,
            digits_compute=dp.get_precision("Account"),
            string="Ready",
            multi="sums",
        ...
        "amount_invalid": fields.function(
            _amount_all,
            digits_compute=dp.get_precision("Account"),
            string="Invalid",
            multi="sums",
        ...
        # Other Info
        "commission_rule_id": fields.function(
            _get_other_info,
            type="many2one",
            relation="commission.rule",
            string="Applied Commission",
            store=False,
            multi="other_info",
        ),
        "last_pay_date_rule": fields.function(
            _get_other_info,
            type="selection",
            selection=LAST_PAY_DATE_RULE,
            string="Last Pay Date Rule",
            store=False,
            multi="other_info",
        ),
        "require_paid": fields.function(
            _get_other_info,
            type="boolean",
        ...
        "require_posted": fields.function(
            _get_other_info,
            type="boolean",
    ...
    _defaults = {"state": "draft", "period_id": _get_period, "name": "/"}
    # _sql_constraints = [
    ...
    def create(self, cr, uid, vals, context=None):
        if vals.get("name", "/") == "/":
            vals["name"] = (
                self.pool.get("ir.sequence").get(cr, uid, "commission.worksheet") or "/"
            )
        ...
        for worksheet in self.browse(cr, uid, ids):
            ...
            if time.strftime("%Y-%m-%d") <= period.date_stop:
                raise osv.except_osv(
                    _("Warning!"),
                    _("You cannot confirm this worksheet. Period not yet over!"),
                )
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        worksheet_line_obj = self.pool.get("commission.worksheet.line")
        for worksheet in self.browse(cr, uid, ids):
            line_ids = worksheet_line_obj.search(
                cr, uid, [("worksheet_id", "=", worksheet.id), ("done", "=", True)]
            )
            if line_ids:
                raise osv.except_osv(
                    _("Warning!"),
                    _(
                        "Worksheet(s) has issued commission(s) and can not be cancelled!"
                    ),
                )
    ...
    def _get_matched_invoices_by_period(
        self, cr, uid, salesperson_id, sale_team_id, period, context=None
    ):
    ...
    def action_calculate(self, cr, uid, ids, context=None):
        ...
        for worksheet in self.browse(cr, uid, ids):
            salesperson_id = (
                worksheet.salesperson_id and worksheet.salesperson_id.id or False
            )
            ...
            if not (salesperson_id or sale_team_id) or not period:
                continue
            rule = (
                worksheet.salesperson_id and worksheet.salesperson_id.commission_rule_id
            ) or (worksheet.sale_team_id and worksheet.sale_team_id.commission_rule_id)
            if not rule:
                raise osv.except_osv(
                    _("Error!"),
                    _("No commission rule specified for this salesperson/team!"),
                )
            # Delete old lines
            line_ids = worksheet_line_obj.search(
                cr, uid, [("worksheet_id", "=", worksheet.id)]
            )
            ...
            invoice_ids = self._get_matched_invoices_by_period(
                cr, uid, salesperson_id, sale_team_id, period, context=context
            )
        ...
        for worksheet in worksheets:
            ...
            if not line_ids:
                raise osv.except_osv(
                    _("Warning!"),
                    _(
                        "No Commission Invoice(s) can be created for Worksheet %s"
                        % (worksheet.name)
                    ),
                )
            ...
            for user in users:
                ...
                for wline in wlines:
                    ...
                    inv_line_rec = invline_obj.product_id_change(
                        cr,
                        uid,
                        [],
                        product_id,
                    ...
                    inv_line_rec.update(
                        {
                            "name": _("Period: ")
                            + worksheet.period_id.name
                            + _(", Invoice: ")
                            + wline.invoice_id.number,
                            "origin": worksheet.name,
                    ...
                    worksheet_line_obj.write(
                        cr,
                        uid,
                        [wline.id],
                        {"done": True, "commission_state": "done"},
                        context,
                    )
            if (
                worksheet_line_obj.search_count(
                    cr,
                    uid,
                    [
                        ("worksheet_id", "=", worksheet.id),
                        ("commission_state", "in", ("draft", "valid")),
                    ],
                )
                <= 0
            ):
...
class commission_worksheet_line(osv.osv):
    ...
    def _get_date_maturity(self, cr, uid, invoice, date_start):
        payment_term_id = (
            invoice.partner_id.property_payment_term
            and invoice.partner_id.property_payment_term.id
            or False
        )
        if payment_term_id:
            pterm_list = self.pool.get("account.payment.term").compute(
                cr, uid, payment_term_id, value=1, date_ref=date_start
            )
            if pterm_list:
    ...
    def _calculate_last_pay_date(self, cr, uid, rule, invoice, context=None):
        if rule == "invoice_duedate":
            ...
            return (
                self._get_date_maturity(cr, uid, invoice, date_start)
                or invoice.date_due
            )
        else:
            return None
    ...
    def _get_invoice_related_payment_amt(self, cr, uid, invoice, context=None):
        ...
        if not ids:
            return 0.0
        cr.execute(
            "select sum(av.amount) from account_voucher av \
                        where av.id in ( \
    ...
    def _get_invoice_related_payment_detail_amt(self, cr, uid, invoice, context=None):
        ...
        if not ids:
            return 0.0
        cr.execute(
            "select sum(pd.amount) from payment_register pd \
    ...
    def _check_commission_line_status(self, cr, uid, line, params, context=None):
        ...
        paid_date = (
            invoice.state == "paid"
            and invoice.payment_ids
            and invoice.payment_ids[-1].date
            or None
        )
        # 2) last_pay_date
        last_pay_date = self._calculate_last_pay_date(
            cr, uid, last_pay_date_rule, invoice, context=context
        )
        # Add buffer
        last_pay_date = (
            datetime.strptime(last_pay_date, "%Y-%m-%d")
            + relativedelta(days=buffer_days or 0)
        ).strftime("%Y-%m-%d")
        # 3) posted payment?
        # 3.1) invoie's payment and paid amount
        posted = (
            invoice.state == "paid" and self._is_pay_posted(cr, uid, invoice) or False
        )
        # 4) is overdue
            ...
            if posted:
                ...
                if not require_posted:
                    if not overdue:
                        ...
                        if allow_overdue:
                            state = (
                                "valid"  # Paid + posted + Overdue, but allow over due
                            )
                        else:
                            state = "invalid"  # otherwise invalid
                else:
    ...
    def update_commission_line_status(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            res = self._check_commission_line_status(
                cr, uid, line, params, context=context
            )
            cr.execute(
                "update commission_worksheet_line set \
                            paid_date = %s, \
                            last_pay_date = %s, \
                            posted = %s, \
                            overdue = %s, \
    ...
    _columns = {
        "worksheet_id": fields.many2one(
            "commission.worksheet", "Commission Worksheet", ondelete="cascade"
        ),
        ...
        "invoice_state": fields.related(
            ...
            readonly=True,
            string="Status",
            selection=[("open", "Open"), ("paid", "Paid"), ("cancel", "Cancelled")],
        ),
        ...
        "overdue": fields.boolean(
            "Overdue", readonly=True, help="For the paid invoice, is it over due?"
        ),
        "commission_state": fields.selection(
            COMMISSION_LINE_STATE, "State", readonly=True
        ),
    ...
    def unlink(self, cr, uid, ids, context=None):
        line_ids = self.search(
            cr, uid, [("id", "in", ids), ("done", "=", True)], context=context
        )
        if line_ids and len(line_ids) > 0:
            ...
            raise osv.except_osv(
                _("Error!"),
                _(
                    "You can't delete this Commission Worksheet, \
                                                because commission has been issued for Invoice No. %s"
                    % (",".join(invoice_numbers))
                ),
            )
        else:
            return super(commission_worksheet_line, self).unlink(
                cr, uid, ids, context=context
            )
...
class res_users(osv.osv):
    ...
    _columns = {
        "commission_rule_id": fields.many2one(
            "commission.rule", "Applied Commission", required=False, readonly=False
        ),
        "require_paid": fields.boolean(
            "Require Paid Invoice", help="Require invoice to be paid in full amount."
        ),
