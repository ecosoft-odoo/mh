...
class print_invoice_list(rml_parse.rml_parse):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if data.get('model') and data['model'] == 'ir.ui.menu':
            ...
            # BKK / Non-BKK
            if data['form']['is_bkk'] == True and data['form']['is_not_bkk'] == False:
                where.append(('is_bangkok_customer', '=', True))
            if data['form']['is_bkk'] == False and data['form']['is_not_bkk'] == True:
                where.append(('is_bangkok_customer', '=', False))

            # Nothing
            if data['form']['is_mh_vat'] == False and data['form']['is_mh_novat'] == False:
                where.append(('is_mh_vat', '=', False))  # This impossible condition ensure nothing return.
                where.append(('is_mh_vat', '=', True))
            if data['form']['is_mh_vat'] == True and data['form']['is_mh_novat'] == False:
                where.append(('is_mh_novat', '=', False))
            if data['form']['is_mh_vat'] == False and data['form']['is_mh_novat'] == True:
                where.append(('is_mh_novat', '=', True))
            # If is_mh_vat = True and is_mh_novat = True, all ok.
...
report_sxw.report_sxw('report.account.invoice.list.report.wzd', 'account.invoice', '', parser=print_invoice_list, header=False)
